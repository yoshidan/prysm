...
// Minimal preset version - CommitteeBits is 1 byte (vs 8 for mainnet)
package eth
...
func (a *AttestationElectra) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (3) 'CommitteeBits' - 1 byte for minimal (vs 8 for mainnet)
	if size := len(a.CommitteeBits); size != 1 {
		err = ssz.ErrBytesLengthFn("--.CommitteeBits", size, 1)
		return
	}
...
func (a *AttestationElectra) UnmarshalSSZ(buf []byte) error {
	...
	if cap(a.CommitteeBits) == 0 {
		a.CommitteeBits = make([]byte, 0, len(buf[228:229]))
	}
	a.CommitteeBits = append(a.CommitteeBits, buf[228:229]...)
